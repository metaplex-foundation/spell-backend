...
// TODO: accept string instead of json value, after json rpc is merged
impl From<(AssetExtended, Value)> for Asset {
    fn from(value: (AssetExtended, Value)) -> Self {
        ...
        Asset {
            ...
            id: crate::l2::pubkey_to_string(l2_asset.pubkey),
            ...
            grouping: l2_asset.collection.map(|c| {
                vec![Group {
                    ...
                    group_value: Some(crate::l2::pubkey_to_string(c)),
...
mod test {
    ...
    fn test_parse_metadata() {
        ...
        let asset_ex = AssetExtended {
            ...
            metadata_uri: "http://link/to/metadata.json".to_string(),
            royalty_basis_points: 0,
        };

        let json_value: Value = serde_json::from_str(&json).unwrap();
        let dto: Asset = (asset_ex, json_value).into();

        // We take name from L2 asset entity, not from metadata JSON
        assert_eq!(
            dto.content
                .as_ref()
                .unwrap()
        ...
        assert_eq!(dto.content.as_ref().unwrap().metadata.get_item("description").unwrap(), "test description");
        assert_eq!(
            dto.content
                .as_ref()
                .unwrap()
        ...
        assert_eq!(
            dto.content.as_ref().unwrap().files.as_ref().unwrap()[0]
                .uri
                .as_ref()
                .unwrap(),
            "http://host/image.png"
