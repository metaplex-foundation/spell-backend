...
impl HdWalletProducer {
    ...
    pub fn make_hd_wallet(&self, account: u32, address: u32) -> Keypair {
        ...
        // At this point safe to unwrap()
        Keypair::from_seed_and_derivation_path(&self.seed, Some(derivation_path)).unwrap()
    }

    // TODO: delete mocks
    pub fn mocked() -> HdWalletProducer {
        let seed = generate_seed_from_seed_phrase_and_passphrase("seed_phrase", "passphrase");
        HdWalletProducer { seed }
    }
}
...
fn test_hd_wallet_generation() {
...

    {
        ...
        assert_eq!(result, sut.make_hd_wallet(1, 1));
    }
    {
        let absolute_str = "m/44'/501'/1'/0/5";
        let derivation_path = DerivationPath::from_absolute_path_str(absolute_str).unwrap();
        let result = Keypair::from_seed_and_derivation_path(&seed, Some(derivation_path)).unwrap();
        assert_eq!(result, sut.make_hd_wallet(1, 5));
    }
    {
        let absolute_str = "m/44'/501'/999'/0/999";
        let derivation_path = DerivationPath::from_absolute_path_str(absolute_str).unwrap();
        let result = Keypair::from_seed_and_derivation_path(&seed, Some(derivation_path)).unwrap();
        assert_eq!(result, sut.make_hd_wallet(999, 999));
    }
}
